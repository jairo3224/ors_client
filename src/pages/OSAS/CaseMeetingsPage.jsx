...
const MOCK_MEETINGS = [
  { id: 1, title: 'Disciplinary Hearing - Juan dela Cruz', date: '2026-06-05', time: '09:00', type: 'Disciplinary Hearing', status: 'Scheduled', participants: 'OSAS, Student, Parent, Teacher', location: 'OSAS Conference Room', notes: 'Review behavioral incident and determine appropriate sanction.', outcome: '' },
  { id: 2, title: 'Case Conference - Maria Santos', date: '2026-06-02', time: '14:00', type: 'Case Conference', status: 'Scheduled', participants: 'OSAS, Guidance, Dept Head', location: 'Guidance Office', notes: 'Discuss academic probation terms and support plan.', outcome: '' },
  { id: 3, title: 'Referral Review Meeting', date: '2026-05-28', time: '10:00', type: 'Review', status: 'Completed', participants: 'OSAS, Guidance, Chaplain', location: 'OSAS Office', notes: 'Reviewed pending referrals and assigned to appropriate offices.', outcome: 'All 12 pending referrals reviewed and routed.' },
  { id: 4, title: 'Expulsion Hearing - Dante Villanueva', date: '2026-05-30', time: '09:00', type: 'Disciplinary Hearing', status: 'Completed', participants: 'OSAS, Dean, Student, Parent', location: 'Board Room', notes: 'Heard testimony and evidence regarding repeated offenses.', outcome: 'Decision pending further investigation. Referred to disciplinary committee.' },
  { id: 5, title: 'Policy Planning - Sanction Guidelines', date: '2026-06-10', time: '15:00', type: 'Planning Session', status: 'Scheduled', participants: 'OSAS, Dean', location: 'Dean\'s Office', notes: 'Review and update student sanction guidelines for the school year.', outcome: '' },
];

const BLANK_FORM = { title: '', date: '', time: '', type: 'Case Conference', status: 'Scheduled', participants: '', location: '', notes: '', outcome: '' };
const TYPE_OPTIONS = ['Case Conference', 'Disciplinary Hearing', 'Review', 'Planning Session', 'Parent Meeting', 'Staff Meeting'];
const STATUS_OPTIONS = ['Scheduled', 'Completed', 'Cancelled', 'Rescheduled'];

const TYPE_COLORS = {
  'Case Conference': 'bg-blue-500',
  'Disciplinary Hearing': 'bg-red-500',
  'Review': 'bg-purple-500',
  'Planning Session': 'bg-green-500',
  'Parent Meeting': 'bg-amber-500',
  'Staff Meeting': 'bg-gray-500',
};
  ...
  return (
    <div className="space-y-4">
      <div className="page-header">
        ...
        <p className="page-subtitle">Schedule and document all case meetings, hearings, and reviews</p>
      </div>
          ...
          <div key={meeting.id} className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-sm transition-shadow">
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-start gap-3">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-white text-sm font-bold ${TYPE_COLORS[meeting.type] || 'bg-gray-500'}`}>
                  <Users size={18} />
                </div>
                <div>
                  ...
                  {meeting.location && <p className="text-xs text-gray-400 mt-0.5">📍 {meeting.location}</p>}
                </div>
              </div>
      ...
      <div className="space-y-3 text-sm">
        <div>
          ...
          <input value={form.title} onChange={e => set('title', e.target.value)}
            className="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-200" placeholder="e.g., Disciplinary Hearing - Student Name" />
        </div>
        ...
        <div>
          ...
          <input value={form.participants} onChange={e => set('participants', e.target.value)}
            className="w-full border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-200"
            placeholder="e.g., OSAS, Student, Parent, Teacher" />
        </div>
        <div>
