...
const MOCK = [
  { id: 1, student_name: 'Carlo Mendoza',   referred_by: 'Ms. Torres',      urgency: 'Low',      status: 'Open',        date: '2026-05-14', description: 'Needs spiritual guidance and personal counseling.', notes: '' },
  { id: 2, student_name: 'Bianca Reyes',    referred_by: 'Mr. Lim',         urgency: 'Moderate', status: 'In Progress', date: '2026-05-20', description: 'Grief and loss support following family bereavement.', notes: 'Initial pastoral visit completed. Student open to continued support.' },
  { id: 3, student_name: 'Dante Villanueva', referred_by: 'Ms. Cruz',       urgency: 'Urgent',   status: 'Open',        date: '2026-05-28', description: 'Personal crisis requiring immediate pastoral intervention.', notes: '' },
  { id: 4, student_name: 'Elena Martinez',  referred_by: 'Mr. Garcia',      urgency: 'Moderate', status: 'Resolved',    date: '2026-05-10', description: 'Faith and spiritual identity exploration.', notes: 'Four sessions completed. Student reports renewed sense of purpose.' },
];
...
export default function ChaplainReferralsPage() {
  ...
  return (
    <div className="space-y-4">
      <div className="page-header">
        ...
        <p className="page-subtitle">Manage referrals assigned to Chaplain's Office</p>
      </div>
      ...
      {modal?.type === 'view' && (
        <Modal title="Referral Details" onClose={() => setModal(null)}>
          <ViewContent referral={modal.data} onStatusChange={handleStatusChange} onAddNote={handleAddNote} onClose={() => setModal(null)} />
        </Modal>
      )}
    </div>
  );
...
function ViewContent({ referral, onStatusChange, onAddNote, onClose }) {
  ...
  return (
    <>
      <dl className="space-y-3 text-sm">
        ...
        {referral.notes && (
          <div className="flex gap-2">
            <dt className="text-gray-500 w-28 shrink-0">Pastoral Notes</dt>
            <dd className="font-medium text-gray-800 whitespace-pre-line">{referral.notes}</dd>
          </div>
        )}
        <div className="flex gap-2 items-center">
      ...
      <div className="mt-4 pt-4 border-t border-gray-100">
        <label className="block text-sm font-medium text-gray-600 mb-2">Add Pastoral Care Note</label>
        <div className="flex gap-2">
          <textarea value={noteText} onChange={e => setNoteText(e.target.value)} rows={2}
            ...
            placeholder="Document your pastoral session notes…" />
          <button onClick={() => { if (noteText.trim()) { onAddNote(referral.id, noteText.trim()); setNoteText(''); } }}
      ...
      <div className="mt-5 flex justify-end gap-2">
        ...
        <button onClick={() => { onStatusChange(referral.id, status); onClose(); }}
          style={{ background: '#4a7c8a' }}
